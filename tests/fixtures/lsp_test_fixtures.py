...
# Configuration
# CI environments need more time - default to 30s in CI, 15s locally
DEFAULT_TIMEOUT = "30.0" if os.getenv("CI") else "15.0"
LSP_READINESS_TIMEOUT = float(
    os.getenv("LSP_READINESS_TIMEOUT", DEFAULT_TIMEOUT)
)  # Per file timeout
LSP_VERBOSE = os.getenv("LSP_VERBOSE", "0") == "1" or bool(
    os.getenv("CI")
)  # Auto-enable in CI
...
async def nvim_client_with_lsp(language_project_dir, language_config):
    ...
    try:
        ...
        # No arbitrary delays needed - LSP is verified ready with actual requests
        yield nvim_client
...
async def nvim_client_with_lsp_basic(language_project_dir):
    ...
    try:
        ...
        source_files = (
            list(language_project_dir.glob("*.py"))
        ...
        if source_files:
            lsp_ready = await wait_for_lsp_ready(
                nvim_client, str(source_files[0]), timeout=15.0
            )
