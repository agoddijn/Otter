...
async def test_breakpoint_pauses_execution(debug_service):
    ...
    assert session_status.status == "paused", (
        f"Should be paused at breakpoint, got: {session_status.status}"
    )
    assert session_status.pid is not None, (
        f"Should still have PID when paused, got: {session_status.pid}"
    )
    ...
    assert len(state["stack_frames"]) > 0, (
        f"Should have at least one stack frame, got: {len(state.get('stack_frames', []))}"
    )
    ...
    assert top_frame.line in [5, 6, 7], (
        f"Top frame should be near our breakpoint, got line: {top_frame.line}"
    )
...
async def test_stop_on_entry(debug_service):
    ...
    assert state.current_line in [2, 3], (
        f"Should be at start of file, got line {state.current_line}"
    )
...
async def test_step_over(debug_service):
    ...
    assert state2.current_line > line1, (
        f"Should advance to next line: {line1} -> {state2.current_line}"
    )
...
async def test_breakpoint_without_stop_on_entry(debug_service):
    ...
    assert len(state.stack_frames) > 0, (
        f"Expected stack frames, got: {len(state.stack_frames)}"
    )
    assert state.current_file is not None, (
        f"Expected current file, got: {state.current_file}"
    )
