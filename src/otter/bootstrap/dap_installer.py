...
DAP_ADAPTER_INFO: Dict[str, Dict[str, Any]] = {
    "python": {
        "name": "debugpy",
...
def check_dap_adapter(
    language: str, runtime_path: Optional[str] = None
) -> DAPAdapterStatus:
...
async def ensure_dap_adapter(
    ...
    status, error_msg = await check_and_install_dap_adapter(
        language, auto_install, runtime_path=runtime_path
    )
